...
import { averageSpeakingConfidence, orderChoiceOptions, scoreWithSupport, spellingIsCorrect } from "./learning-integrity";
...
type Profile = {
  ...
  createdAt: string;
  savedAt: string;
  diagnostic?: { score: number; band: "Gỡ nút" | "Vừa sức" | "Bứt phá"; takenAt: string; measurementVersion: 3 };
...
function Recorder({ onPractised }: { onPractised: (confidence: number) => void }) {
  ...
  return (
    <div className="recorder">
      <div className="record-actions"><Button variant="outline" onClick={recording ? stop : start}>{recording ? <><Square /> Dừng ghi</> : <><Mic /> Ghi giọng của con</>}</Button>{audioUrl && <audio controls src={audioUrl} aria-label="Nghe lại giọng vừa ghi" onPlay={() => setReviewed(true)} />}</div>
      {error && <div className="record-fallback"><p className="inline-note">{error}</p><button onClick={() => setSpokenWithoutRecording(true)}><Mic /> Con đã nói thành tiếng</button></div>}
      <p className="privacy-note"><LockKeyhole /> Bản ghi chỉ ở tạm trên thiết bị và không được tải đi.</p>
      {reviewed || spokenWithoutRecording ? <div className="self-check"><span>{reviewed ? "Sau khi nghe lại, con thấy:" : "Sau khi tự nghe giọng mình, con thấy:"}</span><button onClick={() => onPractised(55)}>Cần thử lại</button><button onClick={() => onPractised(78)}>Đã nói trọn ý</button><button onClick={() => onPractised(92)}>Rõ và tự tin</button></div> : <p className="review-gate"><Headphones /> {audioUrl ? "Bấm nghe lại bản ghi để mở phần tự đánh giá." : "Ghi âm, nói xong và nghe lại trước khi tự đánh giá."}</p>}
    </div>
  );
}
...
function Challenge({ week, lessonId, sessionIndex, step, band, onDone }: ChallengeProps) {
  ...
  if (sessionIndex === 1) {
    ...
    const phrase = step === 1 ? target.en : week.model;
    const visualWord = step === 1 ? target : modelWord;
    return <div className="challenge speaking-challenge"><p className="challenge-kicker">Nghe → nhẩm → nói</p><VocabularyArt symbol={visualWord.icon} label={visualWord.vi} size="large" /><h2>{step === 1 ? target.en : "Câu mẫu trọn ý"}</h2><button className="listen-orb" onClick={() => speak(phrase)}><Volume2 /><span>Nghe mẫu</span></button><p className="big-phrase">{phrase}</p><p className="tip">{step === 1 ? "Nghe kỹ, che chữ rồi gọi lại từ bằng tranh." : "Nghe một lần, nhìn sang chỗ khác, nói cả câu; sau đó đổi một chi tiết để câu mang ý của con."}</p><Recorder onPractised={(confidence) => onDone(confidence >= 75 ? 100 : 70, confidence, visualWord.en)} /></div>;
  }
...
function AssessmentView({ setProfile, navigate }: { setProfile: React.Dispatch<React.SetStateAction<Profile>>; navigate: (view: View) => void }) {
  ...
  return <div className="page assessment-page"><div className="assessment-top"><div><p className="eyebrow">Không tính điểm thi</p><h1>Khám phá cách con đang học</h1></div><span>{index + 1}/{diagnosticItems.length}</span></div><Progress value={(index / diagnosticItems.length) * 100} /><div className="assessment-card"><p>{item.prompt}</p>{item.mode === "listen" && <button className="listen-orb compact" onClick={() => { setHeard(true); speak(item.spoken ?? "", true); }}><Play /><span>{heard ? "Nghe lại" : "Nghe"}</span></button>}<div className="choice-list">{item.options.map((option) => <button key={option} disabled={item.mode === "listen" && !heard} className={selected === option ? (option === item.answer ? "correct" : "wrong") : ""} onClick={() => answer(option)}>{option}</button>)}</div>{item.mode === "listen" && !heard && <p className="tip">Nghe trước để mở khóa các lựa chọn.</p>}{selected && <p className={selected === item.answer ? "feedback success" : "feedback try"}>{selected === item.answer ? "Con nghe/đọc đúng rồi." : `Đáp án phù hợp là: ${item.answer}`}</p>}<Button disabled={!selected} onClick={next}>{index === diagnosticItems.length - 1 ? "Xem điểm bắt đầu" : "Câu tiếp theo"} <ArrowRight /></Button></div></div>;
}
